...
2020 Erkek verileri : Poverty Rate vs Homicide
...
correlation_result <- cor.test(data_son_temiz_erkek$`Poverty Rate (%)`, data_son_temiz_erkek$Bribery, method = "pearson")
...
b <- ggplot(data_son_temiz_kadin, aes(x = `Poverty Rate (%)`, y = Homicide, color = Region)) +
  geom_point(size = 5) +  # Noktaları boyutlandırma
  geom_smooth(method = "lm", se = FALSE, color = "black") + 
  theme_minimal() +
...
```{r}
# Pearson korelasyonu hesaplama(Povery Rate vs Homicide -Female 20)
...
```{r}
# Pearson korelasyonu hesaplama(Povery Rate vs Homicide -Male 19)
...
```{r}
# Pearson korelasyonu hesaplama(Povery Rate vs Homicide -Female 19)
...
2020 Erkek Verileri-Poverty Rate vs Bribery
...
```{r}
# Pearson korelasyonu hesaplama(Povery Rate vs Bribery -Male 20)
...
2020 Kadın Verileri-Poverty Rate vs Robbery
...
2019 Erkek Verileri-Poverty Rate vs Robbery
...
b <- ggplot(data_son_temiz_erkek_19, aes(x = `Poverty Rate (%)`, y = Robbery, color = Region)) +
  ...
  labs(title = "Poverty Rate vs Robbery -Male19",
       ...
       y = "Suç Oranı") +
  scale_x_continuous(limits = c(4, 16)) +  # X ekseni limitleri
  scale_y_continuous(limits = c(0, 3300)) +  # Y ekseni limitleri
  geom_label_repel(aes(label = Region_Number), size = 3, max.overlaps = 50) +  # Etiketler için
  theme_fivethirtyeight()  # Grafik teması
...
```{r}
# Pearson korelasyonu hesaplama(Povery Rate vs Robbery -Female 29)
...
2019 Kadın Verileri-Poverty Rate vs Robbery
